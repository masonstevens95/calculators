...
import type { KitTier } from './models';
...
export function OlaminaDscrComponent() {
  ...
  function setHomeCount(count: number) {
    ...
    setInputs((prev) => {
      ...
      while (next.length < clamped) next.push({ modelId: 'birmingham' });
      next.length = clamped;
  ...
  function setHomeModel(index: number, modelId: string) {
    setInputs((prev) => ({
      ...prev,
      homes: prev.homes.map((h, i) => (i === index ? { ...h, modelId } : h)),
    }));
  }
  ...
  function setHomeBuildOverride(index: number, override: number | '') {
    setInputs((prev) => {
      const next = prev.homes.map((h, i): HomeInput => {
        if (i !== index) return h;
        if (override === '') {
          const { buildOverride: _drop, ...rest } = h;
          return rest;
        }
  ...
  return (
    <section className={styles.layout} aria-labelledby="olamina-heading">
      <div className={styles.header}>
        <h1 id="olamina-heading" className={styles.heading}>
          Olamina DSCR Calculator
        </h1>
        <p className={styles.subtitle}>
          Per-home DSCR analysis for an Olamina (Momo Homes) build portfolio with kit-tier
          pricing.
        </p>
      ...
      <fieldset className={styles.card}>
        ...
        <FormField label="Kit tier">
          {({ id }) => (
            ...
            >
              <option value="plus">Momo Plus</option>
              <option value="max">Momo Max</option>
            </select>
          )}
        </FormField>
      ...
      <div className={styles.paramGrid}>
        <fieldset className={styles.card}>
          ...
          <FormField label="Down payment (%)">
            {({ id, describedBy }) => (
              <PercentInput id={id} aria-describedby={describedBy} value={inputs.downPct} onChange={setField('downPct')} />
            )}
          </FormField>
          <FormField label="DSCR target">
            {({ id, describedBy }) => (
              <NumberInput id={id} aria-describedby={describedBy} value={inputs.dscrTarget} onChange={setField('dscrTarget')} />
            )}
          </FormField>
        ...
        <fieldset className={styles.card}>
          ...
          <FormField label="Total infrastructure">
            {({ id, describedBy }) => (
              <CurrencyInput id={id} aria-describedby={describedBy} value={inputs.infraTotal} onChange={setField('infraTotal')} />
            )}
          </FormField>
          <FormField label="Property tax (%/yr)">
            {({ id, describedBy }) => (
              <PercentInput id={id} aria-describedby={describedBy} value={inputs.taxRatePct} onChange={setField('taxRatePct')} />
            )}
          </FormField>
        ...
        <fieldset className={styles.card}>
          ...
          <FormField label="Origination fee (%)">
            {({ id, describedBy }) => (
              <PercentInput id={id} aria-describedby={describedBy} value={inputs.origFeePct} onChange={setField('origFeePct')} />
            )}
          </FormField>
      ...
      <AriaLive className={styles.results}>
          ...
          <table className={styles.breakdownTable}>
            <thead>
              <tr>
                ...
                <th>Sqft</th>
                <th>BR/BA</th>
                <th>Kit ({inputs.kitTier === 'plus' ? 'Plus' : 'Max'})</th>
                <th>Build</th>
                <th>Total</th>
                <th>Down</th>
                <th>PITIA/mo</th>
        ...
        <div className={styles.summaryGrid}>
          ...
          <ResultDisplay label={`Total down (${inputs.downPct}%)`} value={formatCurrency(totals.down)} />
          <ResultDisplay
...
export default OlaminaDscrComponent;
